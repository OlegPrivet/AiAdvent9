...
Окно контекста определяется только выбранной моделью из `src/config.rs`. Отдельный лимит приложения удалён; старое поле `context_tokens` игнорируется при загрузке и больше не сохраняется или отправляется API. Неизвестное окно модели вызывает ошибку до HTTP-запроса. `agi` сохраняет `usage` каждого ответа API. Фактическое заполнение окна равно `usage.total_tokens` последнего основного вызова, а накопленный расход диалога равен сумме входных, выходных и общих токенов всех сохранённых вызовов, включая агентов и суммаризацию. Эти величины не смешиваются: накопленный расход повторно учитывает передаваемую историю и потому не сравнивается с размером окна. Если последний основной вызов использовал не менее 80% окна, перед следующим вопросом `agi` суммаризирует всю завершённую историю выбранной моделью. При отсутствии `usage` или после смены модели приложение не угадывает число токенов; допустимость нового запроса определяет NeuralDeep. `/summarize` и `/суммаризация` запускают операцию вручную. Суммаризация использует обычный `/chat/completions` с SSE и usage, без tools, пользовательского Structured Output и stop-последовательностей; лимит генерации до 8192 токенов, не более половины окна. Большая история обрабатывается частями с последующим объединением резюме. После проверки результата резюме атомарно заменяет исходные сообщения в SQLite (схема v4, `chats.summary_json`), затем новый вопрос отправляется с резюме как данными предыдущего диалога. Ошибка/отмена до сохранения не меняет историю; сообщения молча не удаляются. Метрики дополнительных вызовов сохраняются локально, не добавляя нестандартных полей в API. Суммаризация — функция `agi`, не встроенная серверная функция NeuralDeep.

`agi` использует описанный ниже стандартный протокол и регистрирует собственный caller-tool `delegate_task(handle, task)`; это инструмент приложения, а не встроенная функция NeuralDeep. `handle` выбирается из глобального каталога `/agents`, `task` — непустая подзадача до 16 000 символов. Assistant-сообщение с `tool_calls` сохраняется во временном контексте, результаты возвращаются сообщениями `role: "tool"` с исходным `tool_call_id` и JSON `{ "ok": true, "content": "…", "truncated": false }` либо `{ "ok": false, "error": "…" }`.
