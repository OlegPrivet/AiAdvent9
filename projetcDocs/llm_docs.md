...
Локальный бюджет контекста по умолчанию — 200 000 токенов, включая резерв `max_tokens` ответа (по умолчанию 10 000). Старые настройки без `context_tokens` получают это значение при чтении. Перед отправкой консервативная оценка по байтам UTF-8, схеме инструментов и запасу шаблона исключает старые полные обмены из запроса, но не из базы. Системные сообщения и текущий обмен с tool calls не обрезаются; их переполнение возвращает локальную ошибку. Это не точная токенизация и не увеличение окна модели на сервере. Поле `context_tokens` хранится только локально и не отправляется API.
